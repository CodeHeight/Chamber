...
using Chamber.Web.Application.StorageProviders;
...
namespace Chamber.Web.Application
{
    public static class AppHelpers
    {
        ...
        {
            return SiteConstants.Instance.ChamberVersion;
        }
        ...
        public static string MemberImage(string avatar, string email, Guid userId, int size)
        {
            ...
            return StringUtils.GetGravatarImage(email, size);  //email is not needed.
        }

        public static string BusinessImage(string avatar, Guid businessId, int size)
        {
            if (!string.IsNullOrEmpty(avatar))
            {
                // Has an avatar image
                var storageProvider = StorageProvider.Current;
                return storageProvider.BuildFileUrl(businessId, "/", avatar, string.Format("?width={0}&crop=0,0,{0},{0}", size));
            }
            //have default business logo
            return StringUtils.GetGravatarLogo(size);
        }
        ...
        public static UploadFileResult UploadFile(HttpPostedFileBase file, string uploadFolderPath, bool onlyImages = false)
        {
            ...
            if (fileName != null)
            {
                ...
                if (file.ContentLength > Convert.ToInt32(SiteConstants.Instance.FileUploadMaximumFileSizeInBytes))
                {
                    ...
                    upResult.ErrorMessage = "File too big.";
                    return upResult;
                }
                ...
                if (!string.IsNullOrEmpty(allowedFileExtensions))
                {
                    ...
                    if (string.IsNullOrEmpty(fileExtension))
                    {
                        ...
                        upResult.ErrorMessage = "Upload file error";
                        return upResult;
                    }
                    ...
                    if (!allowedFileExtensionsList.Contains(fileExtension))
                    {
                        ...
                        upResult.ErrorMessage = "File extension not allow";
                        return upResult;
                    }
                }
